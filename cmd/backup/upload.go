...
If no name is specified, the filename will be used as the backup name.
...
Examples:
  ...
  pb backup upload ~/Downloads/backup_2024_01_15     # Upload from different location`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		backup, err := client.UploadBackup(filePath, backupName, progressCallback)
		if err != nil {
...
func init() {
	uploadCmd.Flags().StringVar(&nameFlag, "name", "", "Custom backup name (uses filename if not specified)")
}
